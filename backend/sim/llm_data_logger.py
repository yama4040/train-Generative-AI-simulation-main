...
1. 安全性 (Survival) - 最優先事項
   ...
   - 駅での停止位置は所定停止位置の±1m以内に収まることが望ましいです。1m以上の誤差がある場合は大きな減点となります。
...
3. 快適性 (Confidence) - 滑らかな走行
   ...
   - 一般的に、頻繁なノッチの切り替えは乗客にとって不快なだけでなく、機器への負担も大きいため、安定した操作を高く評価してください。

#運転手の好み
- 運転の基本操作は駅から加速を開始し，定時到着するために必要な速度まで加速した後は，惰行を活用し無駄な消費エネルギーを抑えます．（惰行により速度が落ちることを考慮しかつ定時運行を実現するため，惰行するタイミングを考えます）
- 駅減速時や，制限速度接近による減速は一発で適切なブレーキノッチに入れることを好みます．何度もノッチを切り替えるのは好みません．
- 駅間が長い場合は惰行後に再度加速する場合もあります．これは定時運行に必要な場合行うことが多いです．
- 前方に下り勾配が接近している場合や，下り勾配を走行中の場合は，惰行を活用することで速度を維持する場合があります．これにより無駄な加速と減速を抑制します．
- 上り勾配走行中などにより速度が落ちた場合は，定時運行に必要な速度を維持するために、惰行から力行に切り替える場合もあります．ただし，何度もノッチを切り替えるのは好まないため、できるだけ安定した操作を心がけます。
"""
...
- 前方の制限情報: {features['next_limit_info']}
- 前方の勾配情報: {features['next_gradient_info']}
- 列車の減速度：4.0 km/h/s（ブレーキ性能）
"""
        output_format = """
# 推論と出力の指示
まず「reason」にて、残り距離・時間から必要な速度を推算し、"Tri-Drive原則"や"運転手の好み"に基づき現在の速度、制限速度、前方制限、駅での減速、惰行の有効性をすべて加味した上で、現在の運転操作が妥当かどうかを100文字程度で論理的に説明してください。
